    ...
    public class HttpFileUploader : IFileUploader
    {
        ...
        public HttpFileUploader(UserFile file, string httpHandlerPath, long ChunkSizeMB)
        {
            ...
            if (string.IsNullOrEmpty(httpHandlerPath))
                throw new ArgumentException("HttpHandler가 설정되어있지 않습니다.");
        ...
        private void StartUpload(bool cancel, bool process)
        {
            ...
            httpHandlerUrlBuilder.Query = string.Format("{0}guid={1}&first={2}&last={3}&cancel={4}&process={5}&name={6}{7}{8}&BoardCd={9}&ContentCd={10}",
                ...
                string.IsNullOrEmpty(_contextParam) ? "" : "&context=" + HttpUtility.UrlEncode(_contextParam),
                _file.BoardCd,
                _file.ContentCd);
