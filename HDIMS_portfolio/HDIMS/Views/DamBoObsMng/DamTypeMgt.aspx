...
<script type="text/javascript">
    ...
    Ext.onReady(function () {
        ...
        function setCommit() {
            var storeCount = damTypeStore.getCount();
            for (var i = 0; i < storeCount; i++) {
                record = damTypeStore.getAt(i);
        ...
        function setGroupCDNum() {
            ...
            var storeCount = damTypeStore.getCount();
            for (var i = 0; i < storeCount; i++) {
                record = damTypeStore.getAt(i).data.GROUPCD;
                if (storeCount < 9) {
        ...
        { type: 'ajax',
            api: {
                read: '/DamBoObsMng/GetDamTypeList',
                create: '/DamBoObsMng/InsertDamType',
                update: '/DamBoObsMng/UpdateDamType',
                destroy: '/DamBoObsMng/DeleteDamType'
            },
            reader: {
                type: 'json',
                root: 'Data'
        ...
        var damTypeColumns =
        [
            { header: '코드<span style="color:red;"> *</span>', flex: 0.4, minWidth: 60, dataIndex: 'DAMTYPE', align: 'center',
                field: {
                    xtype: 'textfield',
                    allowBlank: false
                }
            },
            { header: '타입명', flex: 0.8, minWidth: 90, dataIndex: 'DAMTPNM', align: 'center', renderer: setCursor,
                field: {
                    xtype: 'textfield'
                }
            },
            ...
            { header: '댐자료전용여부', width: 100, dataIndex: 'USEYN', align: 'center', renderer: getUseYN
            }
        ];


        /* 그리드 tbar 아이템 정의 */
        var damTypeTbarItems = [
            {
                xtype: 'displayfield',
                id: 'dpf',
                name: 'dispfld2',
            ...
            '->',
            {
                ...
                scope: this,
                handler: function () {
                    var rec = new damTypeModel({
                        GROUPCD: setGroupCDNum(),
                        GROUPNM: '',
                        GROUPEXPLAIN: ''
                    }), edit = damTypeGrid.editing;

                    rec.setDirty();
                    edit.cancelEdit();
                    damTypeStore.insert(0, rec);
            ...
            }, '-',
            {
                ...
                scope: this,
                handler: function () {
                    var selection = damTypeGrid.getSelectionModel().getSelection()[0];
                    if (selection) {
                        damTypeStore.remove(selection);
                    }
                }
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    for (var i = 0; i < storeCount; i++) {
                        record = damTypeStore.getAt(i);
                        if (record.dirty == true) {
                            if (damTypeStore.getAt(i).data.DAMTYPE == '' || damTypeStore.getAt(i).data.DAMTPNM == '') {
        ...
        /* 그리드 정의 */
        var damTypeGrid = Ext.create('Edit.Grid', {
            id: 'damTypeGrid',
            name: 'damTypeGrid',
            frame: true,
            flex: 1,
            region: 'center',
            width: 500,
            columns: damTypeColumns,
            store: damTypeStore,
            tbar: damTypeTbarItems,
            columnLines: true,
            listeners: {
                beforeedit: function (editor, e, options) {
                    var commitStoreCount = damTypeStore.getTotalCount();
                    var totalStoreCount = damTypeStore.getCount();
        ...
        damTypeGrid.on("cellclick", function (gg, rowIndex, colIndex, rec) {
            if (colIndex == 3) {
                var val = rec.get("USEYN");
                if (val == "Y") rec.set("USEYN", "N");
                else rec.set("USEYN", "Y");
            } 
        });
