
namespace HDIMS.Models.Domain.DamBoObsMng
{
    public class DamColMgtModel
    {
        public string DAMCD { get; set; }
        public string DAMNM { get; set; }
        public string RWL { get; set; }
        public string RSQTY { get; set; }
        public string RSRT { get; set; }
        public string OSPILWL { get; set; }
        public string IQTY { get; set; }
        public string ETCIQTY1 { get; set; }
        public string ETCIQTY2 { get; set; }
        public string ETQTY { get; set; }
        public string TDQTY { get; set; }
        public string EDQTY { get; set; }
        public string ETCEDQTY { get; set; }
        public string SPDQTY { get; set; }
        public string OTLTDQTY { get; set; }
        public string ETCDQTY1 { get; set; }
        public string ETCDQTY2 { get; set; }
        public string ETCDQTY3 { get; set; }
        public string ITQTY1 { get; set; }
        public string ITQTY2 { get; set; }
        public string ITQTY3 { get; set; }
    }
}