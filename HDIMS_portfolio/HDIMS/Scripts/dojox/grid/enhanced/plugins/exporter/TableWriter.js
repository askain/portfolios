	...
	dojox.grid.enhanced.plugins.exporter._ExportWriter, {
	    ...
	    handleCell: function (/* object */arg_obj) {
			...
			attrs = [cell.colSpan ? ' colspan="' + cell.colSpan + '"' : '',
					...
					arg_obj.isHeader ? ' style="mso-number-format:\\@;width: ' : ' style="width: ', 
                    dojo.contentBox(cell.getHeaderNode()).w, 'px;"',
	    ...
	    toString: function () {
	        ...
	        return ['<style>td{border:1px solid #626262;} .grid_header{mso-number-format:\\@;color: #000000;background-color: #efefef;font-style: italic;font-weight: bold;text-align:center;}</style><div style="position: relative;">', viewsHTML, '</div>'].join('');
	    }
	});
}
